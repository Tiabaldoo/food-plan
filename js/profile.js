(()=>{
  ...
  function recommendationCards(key){
    ...
    return `<div class="calorie-goals">
      ${['loss','maintain','gain'].map(goal=>`<button type="button" class="calorie-goal ${p.goal===goal?'active':''}" data-profile-goal="${key}|${goal}"><span>${goal==='loss'?'↘':goal==='gain'?'↗':'≈'} ${goalLabels[goal]}</span><strong>${r[goal]||'—'}${r[goal]?' ккал':''}</strong>${goal==='loss'?`<small>дефицит ${data.shared.lossPercent}%</small>`:goal==='gain'?`<small>профицит ${data.shared.gainPercent}%</small>`:'<small>примерный расход за день</small>'}</button>`).join('')}
    </div>`;
  }
  ...
  function personCard(key){
    ...
    return `<article class="profile-card" data-profile-card="${key}">
      <div class="profile-card-head"><div><p class="eyebrow">${personTitle(key)}</p><h2>${esc(p.name)}</h2></div><div class="profile-target-badge"><span>Текущая цель</span><strong>${p.target||'—'} ккал</strong></div></div>
      <div class="profile-fields">
        <label><span>Имя</span><input type="text" maxlength="40" value="${esc(p.name)}" data-profile-field="${key}|name"></label>
      ...
      <section class="profile-calculator">
        <div class="profile-section-title"><div><p class="eyebrow">Мини-калькулятор</p><h3>Сколько калорий в день</h3></div></div>
        ${recommendationCards(key)}
        ${calculationInfo(key)}
        <div class="target-mode">
          <label class="target-mode-option"><input type="radio" name="targetMode-${key}" value="auto" data-profile-mode="${key}" ${p.calorieMode==='auto'?'checked':''}><span><strong>Автоматически</strong><small>Использовать выбранную цель калькулятора</small></span></label>
        ...
        <label class="manual-target ${p.calorieMode==='auto'?'muted':''}"><span>Моя цель, ккал</span><input type="number" min="900" max="6000" step="10" value="${num(p.manualTarget)}" data-profile-field="${key}|manualTarget" ${p.calorieMode==='auto'?'disabled':''}><small>${p.calorieMode==='auto'?`Сейчас применяется расчёт: ${calc.recommendations(p,data.shared)[p.goal]||'—'} ккал`:`Расчёт сайта для «${goalLabels[p.goal].toLowerCase()}»: ${calc.recommendations(p,data.shared)[p.goal]||'—'} ккал`}</small></label>
      </section>
  ...
  function render(){
    root.innerHTML=`<div class="profiles-intro"><div><p class="eyebrow">Наши параметры</p><h2>Профили и цели</h2><p>Изменения профиля влияют только на новые дни. Каждый уже созданный день хранит свою цель калорий отдельно.</p></div></div>
  ...
  function saveAndRefresh(){calc.save();render();if(typeof render==='function'){};if(typeof window.refreshFoodCalendar==='function')window.refreshFoodCalendar();if(typeof window.render==='function')window.render()}

  root.addEventListener('change',e=>{
    const field=e.target.dataset.profileField;
    if(field){const [key,name]=field.split('|');data[key][name]=parseValue(name,e.target.value);calc.save();render();if(typeof window.render==='function')window.render();return}
    ...
    if(shared){data.shared[shared]=Math.max(0,Number(e.target.value)||0);calc.save();render();if(typeof window.render==='function')window.render();return}
    const mode=e.target.dataset.profileMode;
    if(mode){data[mode].calorieMode=e.target.value;calc.save();render();if(typeof window.render==='function')window.render();return}
  });
  root.addEventListener('click',e=>{
    const goal=e.target.closest('[data-profile-goal]');if(!goal)return;
    const [key,value]=goal.dataset.profileGoal.split('|');data[key].goal=value;calc.save();render();if(typeof window.render==='function')window.render();
  });
