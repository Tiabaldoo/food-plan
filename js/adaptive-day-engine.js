(()=>{
  ...
  function proteinTarget(person){const p=window.PROFILES?.[person]||{};const weight=Number(p.weight)||0;const goal=p.goal||'loss';const sex=p.sex||'male';return Math.round(clamp(weight*(PROTEIN_PER_KG[goal]||1.3),MIN_PROTEIN[sex]||75,MAX_PROTEIN[sex]||160))}
  function recipeFor(state,key,type){
    const d=window.MenuEngine.getDay(state,key),list=window.RECIPES?.[type]||[],raw=list[d[type]||0]||null;
    // RECIPES contains the legacy menu objects. RecipeModel.get() is the enriched
    // version carrying adaptive ingredients from the category bridges.
    return raw?(window.RecipeModel?.get(raw.id)||raw):null;
  }
  ...
  function planPerson(state,key,person){
    ...
    const meals={};
    TYPES.forEach(type=>{const recipe=recipeFor(state,key,type);const desired=mealBudget(foodTarget,type);meals[type]={type,recipe,desired,result:recipe?calc(recipe,desired):{ok:false,error:'recipe-not-found'}}});
    for(let pass=0;pass<5;pass++){
      const total=TYPES.reduce((s,t)=>s+(meals[t].result.ok?meals[t].result.nutrition.kcal:0),0),diff=foodTarget-total;if(Math.abs(diff)<=18)break;
      ...
      const weights={breakfast:.8,snack:.5,lunch:1,dinner:.9},sum=candidates.reduce((s,m)=>s+(weights[m.type]||1),0);
      candidates.forEach(m=>{const share=(weights[m.type]||1)/sum,next=Math.round(m.result.nutrition.kcal+diff*share);m.result=calc(m.recipe,next);m.desired=next});
    }
    ...
    const proteinGoal=proteinTarget(person),proteinStatus=nutrition.p>=proteinGoal?'ok':nutrition.p>=proteinGoal*.85?'near':'low';
